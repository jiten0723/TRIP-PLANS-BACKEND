...
const findAll = asyncHandler(async (req, res) => {
  const trips = await Trip.find({
    // Fetch trips where the user is either the owner or a collaborator
...
const addExpenses = asyncHandler(async (req, res) => {
  ...
  trip.budget.spent += req.body.amount;
  await trip.save();
  res.status(201).json(trip);
});
const inviteCollaborator = asyncHandler(async (req, res) => {
  console.log("Inviting collaborator", req.params.id);
  const trip = await Trip.findOne({
    _id: req.params.id,
    user: req.user.userId,
  });
...
// This function will be called when the user clicks on the invitation link
...
const deleteFile = asyncHandler(async (req, res) => {
  ...
  const file = trip.files?.find((file) => file?.publicId === req.query.fileId);
